# -*- coding: utf-8 -*-
"""Modified baseline model code - DS340W

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1DG__TWEcjmxOrZU3GgEA-KdupFFm-fJf
"""

## modified prompt_and_align.py
import torch
...
train_conf, adj = pickle.load(open('data/adjs/user_t' + str(args.u_thres) + '/' + args.dataset_name + '_nn_relations_' + str(args.n_samples) + '.pkl', 'rb'))
A_nn = adj.todense()
...
class NewsDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        label = np.array([1, 0]) if label_logit == 0 else np.array([0, 1])
...
class BERTPrompt(nn.Module):
    ...
    def forward(self, input_ids, masked_position):
        ...
        output = self.bert(input_ids=input_ids, output_attentions=True)
        ...
        attentions = output[-1][-1]  # Last layer attentions
...
# Training function with feature importance and attention maps
def train_model(args, x_train, x_test, y_train, y_test, tokenizer, max_len, n_epochs, batch_size, datasetname, iter):
    ...
    for epoch in range(n_epochs):
        ...
        for batch_data in tqdm(train_loader):
            ...
            out_labels, attentions = model(input_ids=input_ids, masked_position=batch_data["masked_pos"][0].item())
            ...
            for i in range(input_ids.size(1)):
                ...
                permuted_input_ids[:, i] = torch.randint_like(permuted_input_ids[:, i], 0, tokenizer.vocab_size)
                permuted_out_labels, _ = model(input_ids=permuted_input_ids, masked_position=batch_data["masked_pos"][0].item())
                _, permuted_pred = permuted_out_labels.max(dim=-1)
                permuted_correct = permuted_pred.eq(target_logits).sum().item()
                permuted_acc.append(permuted_correct / len(targets))
            feature_importance = [orig_acc - permuted_acc[i] for i, orig_acc in enumerate(avg_acc)]

            # Store feature importance scores and attention maps
            batch_data['feature_importance'] = feature_importance
            batch_data['attentions'] = attentions
        ...
        for batch_data in tqdm(test_loader):
            ...
            with torch.no_grad():
                ...
                test_out, _ = model(input_ids=input_ids, masked_position=batch_data["masked_pos"][0].item())
        ...
        if epoch == n_epochs - 1:
            ...
            precision, recall, fscore, _ = score(y_test.detach().cpu().numpy(), y_pred_upd.detach().cpu().numpy(),
                                                 average='macro')
...
# Load dataset splits
x_train, x_test, y_train, y_test = get_splits_fewshot(args.dataset_name, args.n_samples)
...
# Write logs
with open('logs/log_' + args.dataset_name + '_fewshot_' + str(args.n_samples) + '_samples_' + args.model_name + '_t' + str(args.u_thres) + '.' + 'iter' + str(iterations), 'a+') as f:
